...
const PikuCalendar = ({
  ...
  const getDayClassName = (day: Date, isCurrentMonth: boolean, isFuture: boolean) => {
    ...
    <main {...handlers} className="flex-1 flex flex-col p-2 min-h-0 overflow-hidden">
      ...
      <div className="grid grid-cols-7 flex-1 gap-1 min-h-0">
        {days.map((day, index) => {
          ...
          const canCreate = isMyCalendar && !pikuData && isCurrentMonth && !isFutureDate;
          ...
          return (
            ...
            >
              {pikuData && isCurrentMonth ? (
                ...
                <span
                  className={`${getDayClassName(day, isCurrentMonth, isFutureDate)} font-medium`}
                >
                  {format(day, 'd')}
                </span>
              )}

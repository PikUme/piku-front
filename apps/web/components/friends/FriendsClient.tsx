...
import { getFriendRequests, acceptFriendRequest, rejectFriendRequest } from '@/lib/api/friend';
...
const FriendsClient = () => {
  ...
  const loadMoreRequests = useCallback(async () => {
    ...
    try {
      ...
      setRequests((prev) => {
        ...
        const newRequests = data.requests.filter((req) => !existingUserIds.has(req.userId));
  ...
  const handleAccept = async (userId: string) => {
    ...
    setRequests((currentRequests) => currentRequests.filter((req) => req.userId !== userId));

...
const FriendList = () => {
  ...
  const fetchFriends = useCallback(async (pageNum: number) => {
    ...
    try {
      ...
      setFriends((prevFriends) => (pageNum === 0 ? newFriends : [...prevFriends, ...newFriends]));
  ...
  if (initialLoad && friends.length === 0 && !isLoading) {
    return <div className="text-center text-gray-500 py-10">친구 목록이 비었습니다.</div>;
  }
  ...
  return (
    <>
      ...
      {isLoading && <div className="text-center py-4">친구를 불러오는 중...</div>}

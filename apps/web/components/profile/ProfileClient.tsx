...
interface ProfileClientProps {
  profileData: UserProfileResponseDTO & Partial<Pick<UserProfile, 'userId' | 'friendStatus'>>;
}
...
const ProfileClient = ({ profileData }: ProfileClientProps) => {
  ...
  const handleMonthClick = (month: number) => {
    ...
    router.push(`/profile/${profileData.userId}/calendar?date=${date.toISOString()}`);
  };
  ...
  return (
    <div className="w-full bg-white">
      ...
      <div className="px-6 pb-6 -mt-16">
        ...
        <h2 className="text-center text-2xl font-bold mt-4">{profileData.nickname}</h2>
      ...
      <div className="text-left px-6 py-4 border-t">
        ...
        <div className="relative">
          {profileData.monthlyDiaryCount && profileData.monthlyDiaryCount.length > 1 && (
            <div className="absolute left-2.5 top-2.5 bottom-2.5 w-1 bg-gray-200"></div>
          )}
          <div className="space-y-4">
            {profileData.monthlyDiaryCount && profileData.monthlyDiaryCount.length > 0 ? (
              profileData.monthlyDiaryCount.map((stat: DiaryMonthCountDTO) => (
                <div
                  key={stat.month}
                ...
                >
                  ...
                  <div className="w-full p-4 border rounded-lg shadow-sm bg-white">
                    <div className="flex items-baseline mb-2">
                      <span className="text-lg font-semibold text-gray-800">{stat.month}월</span>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${
                            stat.daysInMonth > 0 ? (stat.count / stat.daysInMonth) * 100 : 0
                          }%`,
                        }}
                      ></div>
                    </div>
              ...
              <p className="text-center text-gray-500">아직 작성된 일기 기록이 없어요.</p>
            )}
          </div>
        </div>
      </div>

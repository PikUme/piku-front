...
import { checkNicknameAvailability, updateUserProfile } from '@/lib/api/user';
...
const ProfileEditClient = ({ profileData }: ProfileEditClientProps) => {
      ...
      try {
        ...
        if (data && Array.isArray(data)) {
          ...
          const currentCharacter = data.find(
            (c) =>
              user?.avatar && stripProtocol(user.avatar).includes(stripProtocol(c.displayImageUrl))
          );
          if (currentCharacter) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const currentCharacter = characters.find(
      (c) => user?.avatar && stripProtocol(user.avatar).includes(stripProtocol(c.displayImageUrl))
    );
    ...
    if (
      selectedCharacter &&
      currentCharacter &&
      String(currentCharacter.id) !== selectedCharacter
    ) {

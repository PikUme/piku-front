...
const HomeCalendar = ({
  ...
  const isMyCalendar = !initialViewedUser || user?.id === initialViewedUser.userId;
  ...
  // 다이어리 데이터 훅
  const { pikus, selectedDiary, isLoading, loadDiaryDetail, closeDiaryDetail } = useDiaryData(
    currentDate,
    user,
    viewedUser
  );

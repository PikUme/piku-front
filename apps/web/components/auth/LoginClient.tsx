...
const LoginClient = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || error.response?.data || '로그인 중 오류가 발생했습니다.';
      ...
      <div className="w-full max-w-md px-8">
        <div className="w-full mb-10">
          <div className="relative text-center">
            <button
              onClick={() => router.back()}
              className="text-2xl font-bold absolute left-0 dark:text-white cursor-pointer"
            >
              &lt;
            </button>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {message && (
            <p
              className={`mt-4 text-center ${message.includes('성공') ? 'text-green-600' : 'text-red-600'}`}
            >
              {message}
            </p>
          )}

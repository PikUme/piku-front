...
const CommentItem = ({
  ...
  const [hoveredElement, setHoveredElement] = useState<'avatar' | 'nickname' | null>(null);
  const [profileCardHoverTimeout, setProfileCardHoverTimeout] = useState<NodeJS.Timeout | null>(
    null
  );
            ...
            {showReplies && (
              <>
                ...
                >
                  ...
                  {isLoading && <p className="text-xs text-gray-400">답글 로딩 중...</p>}
                  {!isLoading && hasMore && (
                    <button

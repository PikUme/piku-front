...
import { diarySchema, DiaryFormValues, UnifiedPhoto, PrivacyStatus } from '@/types/diary';
import {
  X,
  Globe,
  Lock,
...
const PhotoItem = ({
  ...
  return (
    <Reorder.Item
      ...
      onTap={(e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const isControlButton =
          target.closest('[data-drag-handle]') || target.closest('[data-remove-button]');
    ...
    >
      <img src={photo.url} alt="selected photo" className="w-full h-full object-cover" />
      ...
      <main className="flex-grow p-4 overflow-y-auto text-black dark:text-white flex flex-col">
          ...
          >
            {isGeneratingAiPhotos ? (
              ...
              <>
                ...
                <span className="text-[11px] sm:text-xs">(횟수: {remainingRequests ?? '...'})</span>
              </>
            )}
          </button>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 flex flex-col">
          ...
          <div className="self-end text-sm text-gray-500">
            <span className={(contentValue?.length || 0) > 500 ? 'text-red-500' : ''}>

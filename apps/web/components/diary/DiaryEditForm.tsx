...
const DiaryEditForm = ({ initialDiaryData }: DiaryEditFormProps) => {
  ...
  useEffect(() => {
    ...
    const fetchedPhotos: UnifiedPhoto[] = initialDiaryData.imgUrls.map((img, index) => ({
  ...
  return (
    <div className="flex flex-col bg-white dark:bg-black">
      <header className="flex items-center justify-between p-4 border-b bg-white dark:bg-gray-900 dark:border-gray-700">
        <button onClick={() => router.back()} className="cursor-pointer dark:text-white">
          <X size={24} />
        </button>
        <h1 className="text-lg font-semibold dark:text-white">{'일기 수정'}</h1>
        <button
      ...
      <main className="p-4 overflow-y-auto text-black dark:text-white">
        ...
        <div className="flex items-center my-4">
          ...
          />
          <span className="font-semibold dark:text-white">{user?.nickname || 'me'}</span>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 flex flex-col">
          <TextareaAutosize
            {...register('content')}
            minRows={10}
            className="w-full p-2 border-none bg-transparent focus:ring-0 placeholder-gray-400 dark:placeholder-gray-500 text-black dark:text-white rounded-md resize-none flex-grow"
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                {(['PUBLIC', 'FRIENDS', 'PRIVATE'] as PrivacyStatus[]).map((status) => (
                  <button
                    key={status}
                        ...
                        {
                          {
                            ...
                            FRIENDS: '나를 팔로우하는 친구들만 볼 수 있습니다.',
                            PRIVATE: '나만 볼 수 있습니다.',
                          }[status]
                        }
                      </p>
      ...
      {isNotReadyModalOpen && <NotReadyModal onClose={() => setIsNotReadyModalOpen(false)} />}
    </div>
  );
};

...
const BottomNav = () => {
  ...
  const getLinkClass = (path: string, exact = true) => {
    ...
    const baseClass = `flex flex-col items-center text-sm ${isActive ? '' : 'text-gray-400'}`;
  ...
  const getMoreLinkClass = () => {
    const isActive = pathname.startsWith('/profile') || pathname.startsWith('/settings');
  ...
  const getBottomNavClass = () => {
    let baseClass =
      'flex justify-around items-center border-t xl:hidden bg-white dark:bg-black fixed bottom-0 left-0 right-0 z-20';
  ...
  return (
    <>
      ...
      <footer className={`${getBottomNavClass()} bottom-nav`}>
        ...
        <Link href={`/diary/new/${todayDate}`} className={getLinkClass('/diary/new', false)}>

...
const FeedClient = () => {
  ...
  const handleUpdateCommentCount = (count: number) => {
    if (commentModalOpen) {
      ...
      setFeed((prevFeed) =>
        prevFeed.map((post) =>
          post.diaryId === commentModalOpen.diaryId ? { ...post, commentCount: count } : post
        )
      );
    }
  };
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      setFeed((prevFeed) => {
        ...
        const uniqueNewPosts = pageData.content.filter((p) => !existingIds.has(p.diaryId));
  ...
  const handleFriendshipStatusChange = (diaryId: number, newStatus: FriendshipStatus) => {
    setFeed((prevFeed) =>
      prevFeed.map((post) =>
        post.diaryId === diaryId ? { ...post, friendStatus: newStatus } : post
      )
    );
  };
  ...
  return (
    <div className="mx-auto max-w-[600px] space-y-8">
      {feed.map((post, index) => (
        <div key={post.diaryId} ref={index === feed.length - 1 ? lastPostElementRef : null}>
          <FeedCard
            post={post}

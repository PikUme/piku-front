...
import { cancelFriendRequest, deleteFriend, sendFriendRequest } from '@/lib/api/friend';
...
interface FeedCardProps {
  post: FeedDiary;
  onFriendshipStatusChange: (diaryId: number, newStatus: FriendshipStatus) => void;
  onContentClick: () => void;
  onCommentClick: () => void;
  isMobile: boolean;
}
...
const FeedCard = ({
  ...
  const handleFriendAction = async (action: () => Promise<any>, newStatus: FriendshipStatus) => {
  ...
  const renderFriendButton = () => {
    ...
    switch (friendshipStatus) {
      case FriendshipStatus.NONE:
        text = '친구 추가';
        action = () =>
          handleFriendAction(() => sendFriendRequest(post.userId), FriendshipStatus.SENT);
        break;
      ...
      case FriendshipStatus.SENT:
        ...
        action = () => {
          setConfirmModalState({
            actionType: 'cancel',
            onConfirm: () =>
              handleFriendAction(() => cancelFriendRequest(post.userId), FriendshipStatus.NONE),
          });
          setIsConfirmModalOpen(true);
        };
        break;
  ...
  const photoUrl = post.imgUrls?.[currentImageIndex] || 'https://via.placeholder.com/600';
      ...
      <div className="w-full rounded-xl border border-gray-200 bg-white shadow-md p-4 dark:border-gray-700 dark:bg-gray-800">
        <div className="flex items-center justify-between p-3">
          <div className="relative flex items-center">
            ...
            >
              <UserProfile userId={post.userId} nickname={post.nickname} avatar={post.avatar} />
            </div>
            <div className="flex items-center">
              <DotIcon />
              <span className="text-xs text-gray-500" title={new Date(post.date).toLocaleString()}>
            ...
            {isHovering && (
              <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                <ProfileHoverCard
                  ...
                  avatar={post.avatar}
                  onStatusChange={() => onFriendshipStatusChange(post.diaryId, post.friendStatus)}
                />
              </div>
            )}
          </div>
        ...
        <div {...swipeHandlers} className="relative aspect-square w-full">
          <div className="h-full w-full cursor-pointer" onClick={onContentClick}>
            <Image
              src={photoUrl}
              alt="Diary image"
              fill

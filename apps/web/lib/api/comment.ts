...
// 댓글 작성
export const createComment = async (data: CommentCreateRequest): Promise<CommentCreateResponse> => {
  try {
...
export const updateComment = async (commentId: number, content: string): Promise<Comment> => {
  ...
  const response = await api.patch<Comment>(`/comments/${commentId}`, commentUpdateDto);
  return response.data;
};

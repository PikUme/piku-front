...
제1조(목적)
이 약관은 PikUme(이하 ‘회사’ 라고 합니다)가 제공하는 제반 서비스의 이용과 관련하여 회사와 회원과의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
제4조(약관의 효력과 변경)
	1. 이 약관은 PikUme(이)가 제공하는 모든 인터넷서비스에 게시하여 공시합니다. '회사'는 '전자상거래 등에서의 소비자보호에 관한 법률(이하 '전자상거래법'이라 함)', '약관의 규제에 관한 법률(이하 '약관규제법'이라 함),'전자문서 및 전자거래 기본법(이하 '전자문서법'이라 함), 전자금융거래법 , 정보통신망 이용촉진 및 정보보호 등에 관한 법률(이하 '정보통신망법'이라 함), '소비자기본법' 등 관계 법령(이하 '관계법령' 이라 함)에 위배되지 않는 범위 내에서 이 약관을 변경할 수 있으며, 회사는 약관이 변경되는 경우에 변경된 약관의 내용과 시행일을 정하여, 그 시행일로부터 최소 7일 (이용자에게 불리하거나 중대한 사항의 변경은 30 일) 이전부터 시행일 후 상당한 기간 동안 공지하고, 기존 이용자에게는 변경된 약관, 적용일자 및 변경사유(변경될 내용 중 중요사항에 대한 설명을 포함)를 별도의 전자적 수단(전자우편, 문자메시지, 서비스 내 전자쪽지발송, 알림 메시지를 띄우는 등의 방법)으로 개별 통지합니다. 변경된 약관은 공지하거나 통지한 시행일로부터 효력이 발생합니다.
...
제7조(회원가입에 대한 승낙)
	...
	3. 제 1항에 따른 신청에 있어 회사는 서비스 제공에 필요한 경우 전문기관을 통한 실명확인 및 본인인증을 요청할 수 있습니다.
...
제8조(회원정보의 변경)
	...
	2. 회원은 회원가입신청 시 기재한 사항이 변경 되었을 경우 온라인으로 수정을 하거나 전자우편 기타 방법으로 회사에 대하여 그 변경사항을 알려야 합니다.
	3. 제2항의 변경사항을 회사에 알리지 않아 발생한 불이익에 대하여는 회원에게 책임이 있습니다.

제9조(회원정보의 관리 및 보호)
	1. 회원의 아이디(ID)와 비밀번호에 관한 관리책임은 회원에게 있으며, 이를 제3자가 이용하도록 하여서는 안됩니다.
...
제11조(개인정보보호)
	...
	3. 회사는 서비스 이용 통계, 접속자 분석, 서비스 개선 등을 위하여 이용자의 IP 주소, 접속 로그, 이용 패턴 등을 수집할 수 있으며, 이는 개인정보처리방침에 따라 처리됩니다.
...
제13조(서비스의 제공)
	...
	3. 회사가 제공하는 서비스의 내용은 다음과 같습니다.
		가. 일기 작성 기능 및 캐릭터 기반 이미지 생성 서비스, 달력 기반 일기 정리 및 시각화 서비스, 친구 추가 기능을 통한 사용자 간의 상호 교류, 피드 형식의 일기 공유 및 열람 서비스
		나. 개인 캐릭터 설정 및 사용자 맞춤형 콘텐츠 제공, 달력 레이아웃 및 이미지 시각화 최적화, 친구 관계 기반 일기 추천 및 피드 구성, 회원의 활동에 기반한 공지, 알림, 서비스 안내 제공
		다. AI 기반 일기 내용 분석 및 캐릭터 이미지 자동 생성 서비스, 외부 이미지 업로드 기능 및 이미지 관리 도구, 서비스 고도화를 위한 기능 업데이트 또는 외부 연동 서비스 제공 
		라. 일기 비공개/친구공개/전체공개 설정 기능, 대표 일기 자동 설정 및 달력 내 노출 기능, 친구의 달력 열람 및 피드 탐색 기능, 사용자 신고 및 차단 기능
		마. 서비스 이용 통계 및 장기 트렌드 분석을 위한 데이터 수집 및 처리, 서비스 품질 향상을 위한 사용자 행동 분석, 비즈니스 인사이트 도출
...
제15조(서비스의 해제·해지 및 탈퇴 절차)
	1. 이용자가 이용 계약을 해지하고자 할 때는 언제든지 홈페이지 상의 이용자 탈퇴 신청을 통해 이용계약 해지  요청할 수 있습니다. 단, 신규가입 후 일정 시간 동안 서비스 부정이용 방지 등의 사유로 즉시 탈퇴가 제한될 수 있습니다.
	2. 회사는 이용자가 본 약관에서 정한 이용자의 의무를 위반한 경우 등 비정상적인 이용 또는 부당한 이용과 이용자 금지프로그램 사용하는 경우 또는 타인의 명예를 훼손하거나 모욕하는 게시물을 작성한 경우 이용자에게 통지하고, 서비스 이용제한, 계약 해지를 할 수 있다.
...
제18조(정보의 제공 및 광고의 게재)
	1. 회사는 이용자가 서비스 이용 중 필요하다고 인정되는 각종 정보 및 광고를 배너 게재, 전자우편(EMail), 휴대폰 메세지, 전화, 우편 등의 방법으로 이용자에게 제공(또는 전송)할 수 있습니다. 다만, 이용자는 이를 원하지 않을 경우 회사가 제공하는 방법에 따라 수신을 거부할 수 있습니다.

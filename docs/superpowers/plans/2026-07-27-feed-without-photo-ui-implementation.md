...
- `더 보기`가 사라진 뒤에는 같은 카드의 상세 보기 영역으로 포커스를 옮기고, 버튼에 최소 32px 높이를 제공한다.
...
- 펼침 후 포커스는 사라진 `더 보기` 대신 같은 카드의 상세 보기 버튼으로 이동한다.
- `더 보기`는 `min-h-8`로 최소 32px 터치 높이를 확보한다.
...
- 포커스 이동은 기존 상세 보기 callback을 호출하지 않는다.
...
- `더 보기`가 제거된 뒤 같은 카드의 상세 보기 버튼이 포커스를 받는다.

...
## 번호 기반 페이지 조회 계약
- 원댓글, 답글, 친구 목록, 받은 친구 요청, 사용자 검색은 `OffsetPageResponse<T>`를 소비한다. 기존 `Page<T>`는 같은 구조의 호환 타입이다.
- 최상위 `content`, `pageable`, `last`, `totalPages`, `totalElements`, `size`, `number`, `sort`, `first`, `numberOfElements`, `empty`를 유지한다. 페이지 메타데이터를 `page` 하위로 옮기지 않는다.
- `pageable`의 페이지 번호·크기·offset·paged·unpaged·sort와 최상위 sort는 서버 값을 유지한다. 요청은 0부터 시작하고 API별 기존 size·sort를 유지한다.
- 추가 조회 종료는 서버의 최상위 `last`를 따른다. 빈 첫 페이지와 범위를 벗어난 빈 페이지도 성공한 응답으로 처리하며, 목록 길이나 전체 개수로 종료 여부를 새로 계산하지 않는다.
- 친구/받은 요청 API는 서버의 `content`를 화면용 `friends`/`requests`로, `last`를 `hasNext`로 변환하고 `totalElements`를 보존한다. 받은 요청 배지는 현재 로드한 목록 길이 대신 서버의 전체 개수를 사용한다.
- 같은 목록의 요청이 진행 중일 때 반복 스크롤이나 클릭은 추가 요청을 만들지 않는다. 다음 페이지 번호는 성공한 조회 후에만 전진하며 실패한 페이지는 같은 번호로 재시도한다.
- 첫 조회와 스크롤에 따른 추가 조회를 구분하고 마지막 페이지 이후에는 요청하지 않는다. 인접 페이지에서 같은 사용자/댓글이 다시 나타나면 식별자 기준으로 한 번만 표시한다.
- 검색어 변경·비움, 일기 전환, 화면 해제 이후에는 이전 요청의 성공·오류·로딩 완료가 새 목록 상태를 덮어쓰지 않는다. 일기 전환 시 원댓글과 부모별 답글의 페이지 상태를 함께 초기화한다.
- 일기 전환 전에 시작한 댓글 작성·수정·삭제의 완료 처리도 새 일기에 적용하지 않는다. 댓글 수정·삭제 실패를 복구할 때는 해당 항목만 복구하고 요청 대기 중 추가로 조회한 페이지는 보존한다.
- 댓글의 `userId`, `nickname`, `avatar` 및 친구·검색 결과의 `avatar`는 null일 수 있다. 익명 작성자의 이름과 답글 입력에는 `null` 문자열을 표시하지 않는다. 댓글 액션은 기존 서버 권한 필드를 따른다.
- 원댓글 페이지의 `totalElements`는 루트 댓글 수다. 답글을 포함한 일기 전체 댓글 수를 이 값으로 덮어쓰지 않는다.
- 알림의 기존 페이지 표현, 피드와 갤러리의 커서 계약은 각각 유지한다. 기존 백엔드 페이지 JSON과 명시적 Web DTO 응답을 같은 계약으로 소비하므로 프런트·백엔드 동시 배포를 요구하지 않는다.
...
- 일기 상세 경로는 서버에서 경로만 사용해 `PikUme 일기` 제목과 `PikUme에서 일기를 확인해 보세요.` 설명을 제공한다. 루트 제목 템플릿을 덧붙이지 않는다.
- 일기 링크의 canonical과 `og:url`은 아래 공유 URL과 동일하다. OG/Twitter 이미지는 공개 범위·삭제·사진 변경·조회 성공 여부에 관계없이 공통 서비스 이미지를 유지한다. 메타데이터 생성에 일기 조회나 세션 접근을 사용하지 않으며 본문·사진·작성자 정보를 넣지 않는다.

## 일기 공유 계약
- 피드 카드, 직접 상세 페이지, 데스크톱 상세 모달, 모바일 스토리 모달이 같은 공유 훅과 UI를 사용한다. 홈·프로필 캘린더, 프로필 사진 목록과 알림은 기존 상세 모달을 통해 제공한다.
- 공유 아이콘과 상세 메뉴는 `DiaryShareDialog` 선택 모달을 연다. 모바일은 하단, 데스크톱은 중앙에 표시하며 공유 항목은 공통 항목 배열에 추가한다. 모달을 여는 것만으로 공유·복사를 실행하지 않는다.
- `링크 복사`는 Clipboard로 상세 URL만 복사하고, 가로 점 3개 아이콘의 `더보기`는 Web Share를 실행한다. 복사 성공은 토스트로, 실패는 선택 모달 안에서 안내하며 실제 API 호출은 각 항목을 누르는 사용자 이벤트 안에서 시작한다.
- 유효한 일기를 정상 조회했으며 현재 상태가 `PUBLIC`, `ANONYMOUS`, `FRIENDS`일 때 공유를 표시한다. `PRIVATE`는 작성자에게도 아이콘과 공유 메뉴를 숨긴다. 친구 공개는 `작성자의 친구만 볼 수 있는 일기예요.` 안내를 제공하며 확인 단계를 추가하지 않는다.
- URL은 `NEXT_PUBLIC_BASE_URL`의 HTTP(S) origin과 `/diary/{diaryId}`를 결합한다. 미설정 기본값은 `https://www.pikume.com`이며 개발·프리뷰는 해당 프런트엔드 주소를 명시해야 한다. 양의 안전한 정수 ID만 사용하고 현재 페이지의 경로·쿼리·해시나 인증정보를 전달하지 않는다. 잘못된 기준 주소로 공유·복사를 실행하지 않는다.
- 시스템 공유 데이터는 `PikUme 일기` 제목, `PikUme에서 일기를 확인해 보세요.` 문구와 상세 URL만 포함한다. 클립보드에는 URL만 쓴다. 공유 API·토큰·파일·본문 발췌·작성자 정보는 추가하지 않는다.
- `더보기`는 Web Share를 우선하며 `canShare` 미지원이면 `share`를 시도한다. `share` 미지원, `canShare` false/예외 및 공유 실행 오류는 복사를 한 번 시도한다. `AbortError`는 취소로 조용히 종료하고 클립보드를 바꾸지 않는다. `InvalidStateError`는 재조작 안내만 제공한다.
- 실제 복사 성공 후에만 공유 선택 모달을 닫고 화면 하단 중앙에 `링크를 복사했어요.` 토스트를 약 3초 표시한다. 모바일 하단 내비게이션과 안전 영역을 피하고 포커스는 원래 공유 버튼으로 복구한다. 복사 실패 시 모달을 유지하며 시스템 공유 정상 종료에는 전달 완료 안내를 하지 않는다.
- 복사 실패 시 선택 모달 안에서 URL 전체 선택과 닫기를 제공한다. 같은 `링크 복사` 항목으로 재시도하며 자동 재시도는 하지 않는다. 키보드 포커스는 모달 안에 유지하고 닫기·배경 클릭·Escape는 공유 모달만 닫은 뒤 공유 버튼으로 포커스를 복구한다. 공유 선택 모달은 기존 history 상태를 보존한 별도 계층을 추가해 브라우저 뒤로가기도 공유 모달만 먼저 닫는다.
- 공유 진행 상태는 조회 화면 안에 두고 반복 입력을 막는다. 일기 변경·비공개 전환·조회 불가·화면 해제 뒤의 이전 결과는 안내나 추가 복사를 만들지 않는다. 기존 일기 조회·좋아요·댓글·캐시 상태를 변경하지 않는다.
- 요청 처리 중 선택 모달을 닫아도 실제 요청이 끝날 때까지 잠금을 유지한다. 닫힌 요청의 성공·실패 안내와 후속 복사는 억제한다.
- 캘린더를 포함한 상세 모달의 부모는 공유 계층에서 현재 상세 history 상태로 복귀하는 이동에 상세를 유지한다. 캘린더는 상세를 열 때마다 고유 식별자를 부여해 이전에 닫은 상세 항목과 구분한다. 공유 닫기나 복사 성공으로 상세·토스트를 함께 해제하지 않으며, 이전 상세 또는 그 아래 계층으로 뒤로갈 때 현재 상세를 닫는다.
- 링크 수신자의 열람은 기존 상세 조회 API가 현재 공개 범위와 작성자와의 친구 관계로 판단한다. 공유가 공개 범위를 넓히지 않는다. 로그인 후 원래 링크 자동 복귀와 전역 401/403 처리는 기존 흐름을 유지한다.
